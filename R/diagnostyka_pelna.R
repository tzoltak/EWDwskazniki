...
#' @param egzWej nazwa kolumny ramki 'dane', która zawiera wyniki(umiejętności)
#' egzaminu wejściowego.
#' @param egzWyj nazwa kolumny ramki 'dane', która zawiera wyniki(umiejętności)
...
#' @param dysl_wej nazwa kolumny ramki 'dane', która zawiera informacje
#' o dysleksji zgłoszonej dla egzaminu wejściowego.
#' @param dysl_wyj nazwa kolumny ramki 'dane', która zawiera informacje
...
#' @param porownanieModeliPlik ścieżka do pliku do zapisu porównania modeli.
...
#' @param maska wektor boolowski określający, które obserwacje z ramki dane
...
#' @import lme4
#' @export
diagnostyka_pelna <- function(dane, stopnie, typModelu, efektLosowy, egzWej,
                              egzWyj, zmGrupujace, dysl_wej, dysl_wyj,
                              katalogRysunki, porownanieModeliPlik,
  ...
  if (length(stopnie) > 1){
    wlasnosciModeli = lapply(modele, diagnostyka_wielomianow, zmWielomian = egzWej,
                             zmGrupujace = zmGrupujace, folderWykresy = katalogRysunki)
