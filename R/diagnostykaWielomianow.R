...
#' @param wykresyFun funkcja rysująca wykresy. Domyślnie jest to png.
...
diagnostyka_wielomianow <- function(model, zmienna, zmiennaGr = NULL, 
  ...
  if( (!isPoly & !any( ktoraZmienna <- c(zmienna, zmiennaGr) %in% colnames(frame))) 
    ...
    stop("Model był zbudowany na danych, które nie zawierają kolumn: ", c(zmienna, zmiennaGr)[!ktoraZmienna])
  }
  ...
  for(indeksGrupowania in 1:max(1, nrow(grupowanie))){
    ...
    grupa = if( is.null(grupowanie) | nrow(grupowanie) ==0 ){
      rep(TRUE,nrow(frame))
    }else{
  ...
  tab = if(is.null(grupowanie) | nrow(grupowanie)==0){
    data.frame(linkTest, monotonicznosc)
  } else {
    data.frame(grupowanie, linkTest, monotonicznosc)
  }
