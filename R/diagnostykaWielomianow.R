...
diagnostyka_wielomianow <- function(model, zmWielomian, zmGrupujace = NULL,
  ...
  if (class(model)=="lm") {
    ...
    # Z resztami częściowymi sprawa jest troszkę bardziej złożona, bo one muszą przechwycić efekty stałe i losowe pozostałych zmiennych, ale też efekty losowe wielomianu (i grupowania):
  ...
  # obsługa parametrów sterujących rysowaniem - wartości domyślne i ew. ich nadpisywanie/dopisywanie
                 ...
                 function(x, zmGrupujace, zmZal, stopien, wykresyParam, smooothScatterPar) {
                   ...
                   smooothScatterPar$main = paste(paste0("wielomian ", stopien, ". stopnia"),
                                                  ...
                                                  paste0(paste(zmGrupujace,
                                                               apply(x[1, zmGrupujace, drop=FALSE],2, as.character),
                                                               sep=": "),
                                                         collapse=", "))
                   ...
                   lines(wielomianXY$x, wielomianXY$y, col=ifelse(monot[1], 3, 2), lwd=2, lty=1)
                   ...
                   for (i in seq_along(inds[-1])) {
                     if (monot[inds[i] + 1]) {  # następny kawałek rosnący
                       ...
                       lines(wielomianXY$x[inds[i]:(inds[i + 1])], wielomianXY$y[inds[i]:(inds[i + 1])], col=2, lwd=2, lty=1)
                     }
                   }
                   ...
                   if ( !is.null(folderWykresy) ) {
                     ...
                     plik = paste0(zmZal, "_stopien", stopien, "_",
                                   paste0(zmGrupujace, x[1, zmGrupujace], collapse="_"), ".png")
