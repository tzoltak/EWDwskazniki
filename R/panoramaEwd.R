...
#' @description
#' Funkcja rysuje "panoramę polskiej edukacji", czyli wykres rozrzutu szkół ze względu na
#' wartości średnich wyników na wyjściu (oś X) i wartości wskaźników EWD (oś Y).
...
#' @param zapiszPng ciąg znaków - ścieżka do katalogu, w którym mają być zapisane pliki
#' PNG z wykresami lub NULL, jeśli wykresy mają nie być zapisywane
...
panorama_ewd = function(sr, ewd, egzamin, typ_szkoly, wskaznik, lata, zapiszPng=NULL,
  ...
  legend("bottomright", lwd = 2, lty = 1, col = 1:2,
         ...
         title=paste0("r = ", format(sqrt(summary(m)$r.squared), nsmall=3, digits=3)),
         bg="white", cex=0.8)
